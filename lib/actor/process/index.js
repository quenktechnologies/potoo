...
var Process = /** @class */ (function () {
    function Process(module, handle, script) {
        ...
        this.self = function () { return _this.handle.self; };
    }
    ...
    Process.prototype.accept = function (e) {
        if (this.process.isJust()) {
            ...
            this.handle.exec(new scripts_1.AcceptScript(e));
        }
        return this;
    };
...
var handleUnknown = function (p) { return function (m) {
    return p.handle.exec(new scripts_1.AcceptScript(m));
}; };
var handleTell = function (p) { return function (m) {
    return p.handle.exec(new scripts_1.TellScript(m.to, m.message));
}; };
var handleRaise = function (p) { return function (_a) {
    var message = _a.error.message, src = _a.src;
    return p.handle.exec(new scripts_3.RaiseScript("Error message from " + src + ": " + message));
}; };
var handleErrors = function (p) { return function (c) {
    return c.on('error', raise(p));
}; };
var raise = function (p) { return function (e) {
    return p.handle.exec(new scripts_3.RaiseScript(e.message));
}; };
var handleExit = function (p) { return function (c) {
    return c.on('exit', function () { return p.handle.exec(new scripts_2.StopScript(p.self())); });
}; };

    ...
    Process.prototype.run = function () {
        this.handle =
            maybe_1.just(spawn(this))
                ...
                .map(handleExit(this));
        this.system.exec(new route_1.Route(self(this), self(this)));
    };
    return Process;
}());
exports.Process = Process;
var self = function (p) { return p.system.identify(p); };
var spawn = function (p) { return child_process_1.fork(path_1.resolve(exports.SCRIPT_PATH), [], {
    env: {
        POTOO_ACTOR_ID: address_1.getId(self(p)),
...
var handleMessages = function (p) { return function (c) {
    return c.on('message', function (m) { return match_1.match(m)
        ...
        .orElse(function (m) { return p.system.exec(new drop_1.Drop(self(p), self(p), m)); })
        .end(); });
}; };
...
var raise = function (p) { return function (e) {
    return p.system.exec(new raise_1.Raise(e, self(p), self(p)));
}; };
var handleExit = function (p) { return function (c) {
    return c.on('exit', function () { return p.system.exec(new kill_1.Kill(self(p), p)); });
}; };

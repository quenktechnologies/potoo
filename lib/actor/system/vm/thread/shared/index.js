...
var SharedThread = /** @class */ (function () {
    ...
    SharedThread.prototype.wait = function (task) {
        ...
        var onSuccess = function () {
            _this.state = __1.THREAD_STATE_CONTINUE;
    ...
    SharedThread.prototype.restore = function (_a) {
        var _this = this;
        var fun = _a.fun, args = _a.args;
        var frame = new frame_1.StackFrame(this.makeFrameName(fun.name), this.script, this, maybe_1.nothing(), fun.foreign ?
            [op.LDN | this.script.info.indexOf(fun), op.CALL] :
            fun.code.slice());
        frame.data = args.map(function (arg) { return ledger_1.isHeapAddress(arg) ?
            _this.vm.heap.move(arg, frame.name)
            : arg; });
        this.fstack = [frame];
        this.fsp = 0;
        this.rp = 0;
    ...
    SharedThread.prototype.nextFrame = function (rp) {
    ...
    SharedThread.prototype.exec = function (name, args) {
        ...
        this.runner.postJob(new runner_1.Job(this, fun, args));
    };
    return SharedThread;
}());

...
var PVM = /** @class */ (function () {
    ...
    PVM.prototype.kill = function (addr) {
        ...
        addrs.every(function (a) {
            ...
            if (mrun.isJust()) {
                var rtime = mrun.get();
                rtime.terminate();
                _this.trigger(events.EVENT_ACTOR_STOPPED, rtime.context.address);
            }
        });
    };
    /**
    ...
    PVM.prototype.exec = function (i, s) {
        ...
        if (s.immediate === true) {
            ret = maybe_1.just(new thread_1.Thread(this, new heap_1.Heap(), rtime.context).run(s));
        }
        else {
            ...
            while ((!array_1.empty(this.queue)) && this.running) {
                ...
                if (ret.isNothing()) {
                    ...
                    ret = maybe_1.just(runtime.run(script));
                }
                else {
                    runtime.run(script);
                }

...
var checkAddress = function (s, addr) {
    return maybe_1.fromBoolean(!s.state.exists(addr));
};
var generate = function (s, template) { return function (addr) {
    return maybe_1.fromNullable(s.allocate(template))
        .map(function (f) {
        s.state.put(addr, f);
        s.exec(new run_1.Run(exports.RUN_START_TAG, addr, template.delay || 0, function () { return s.state.runInstance(addr); }));
        return f.actor;
    });
}; };

...
var checkAddress = function (s, addr) {
    return maybe_1.fromBoolean(!s.actors.exists(addr));
};
var generate = function (s, template) { return function (addr) {
    return maybe_1.fromNullable(state_1.newFrame(template.create(s), template))
        .map(function (f) {
        s.actors.put(addr, f);
        s.exec(new run_1.Run(exports.RUN_START_TAG, addr, template.delay || 0, function () { return s.actors.runInstance(addr); }));
        return f.actor;
    });
}; };
